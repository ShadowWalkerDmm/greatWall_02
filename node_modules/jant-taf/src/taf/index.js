...
function taf(options) {
    ...
    return (tree) => {
        ...
        if (!tc) {
            console.log("Le fichier de configuration (taf.config.json) est inexistant! Générez le avec votre backend TAF");
            return;
        }
        else {
        ...
        if (!selected_module) { // module introuvable
            console.log("Module introuvable dans le fichier de configuration taf.config.json");
        ...
        if (!options.table) {
            ...
            return (0, schematics_1.schematic)("module", { module: module, projectName: tafConfig.projectName, les_tables: selected_module.les_tables, tafConfig: tafConfig });
        }
        // table précisée
        ...
        if (!selected_table) { // table non retrouvée
            console.log("Table inexistante dans le fichier de configuration taf.config.json");
        ...
        if (!options.type) {
            ...
            return (0, schematics_1.schematic)("module", { module: module, projectName: tafConfig.projectName, les_tables: [selected_table], tafConfig: tafConfig });
        }
        ...
        if (!selected_type) {
            console.log("Type introuvable pour la table " + selected_table.table);
